...
class CustomFlowMatchEulerDiscreteScheduler(FlowMatchEulerDiscreteScheduler):
    ...
    def set_train_timesteps(self, num_timesteps, device, linear=False):
        if linear:
            ...
            t = torch.sigmoid(torch.randn((num_timesteps,), device=device)) # Generate num_timesteps values between 0 and 1
            eps = 1e-6
            # Scale and reverse the values to go from 1000 to 0
            timesteps = (1 - t) * (1000)  # Generate values from 1000 to 0

            # Sort the timesteps in descending order
            timesteps, _ = torch.sort(timesteps, descending=True)   # Sort in descending order
